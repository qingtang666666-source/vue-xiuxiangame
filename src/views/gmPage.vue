...
<script setup>
  ...
  const killWorldBoss = () => { if (!player.value.worldBoss) player.value.worldBoss = { rewards: {} }; player.value.worldBoss.rewards = player.value.worldBoss.rewards || {}; player.value.worldBoss.rewards.kill = (player.value.worldBoss.rewards.kill || 0) + 100000; player.value.worldBoss.hp = 0; player.value.worldBoss.maxHp = player.value.worldBoss.maxHp || 1; player.value.worldBoss.dead = true; note('世界Boss已讨伐(+10万灵石奖励)') }
