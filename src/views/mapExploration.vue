<template>
  <div>
    <div class="legend">
      ...
      <div class="legend-item">
        <div class="color-box herb"></div>
        灵草
      </div>
      <div class="legend-item">
        <div class="color-box treasure"></div>
        宝箱
      </div>
      <div class="legend-item">
        <div class="color-box mine"></div>
        矿脉
      </div>
      <div class="legend-item">
        <div class="color-box portal"></div>
        秘境
      </div>
      <div class="legend-item">
        <div class="color-box lair"></div>
        领地
      </div>
      <div class="legend-item">
...
<script setup>
  ...
  const COLLECTIBLE_TYPES = ['herb', 'treasure', 'mine']
  const TRIGGER_TYPES = ['portal', 'lair', 'event']
  const giftShow = ref(false)
  // NPC的数量
  const npcCount = ref(12)
  // 是否正在长按
  ...
  const initializeGrid = () => {
    ...
    // 生成钓鱼点
    generateItems('fishing', 12, safeZone)
    // 确保钓鱼点生成不会堵塞路径
    ensurePathAvailability()
    // 生成可拾取物：灵草 / 宝箱 / 矿脉
    generateItems('herb', 14, safeZone)
    generateItems('treasure', 9, safeZone)
    generateItems('mine', 10, safeZone)
    generateItems('portal', 3, safeZone)
    generateItems('lair', 4, safeZone)
    generateItems('event', 5, safeZone)
    // 生成NPC
    ...
    while (!openSet.isEmpty()) {
      ...
      for (const [dy, dx] of directions) {
        ...
        if (
          ...
          newX >= 0 &&
          newX < gridSize.value &&
          ['empty', ...COLLECTIBLE_TYPES, ...TRIGGER_TYPES].includes(grid.value[newY * gridSize.value + newX].type)
        ) {
  ...
  const goPlayerXY = (newY, newX) => {
    ...
    // 空地或可拾取物都可踏入；踩到可拾取物则收取
    if (['empty', ...COLLECTIBLE_TYPES, ...TRIGGER_TYPES].includes(grid.value[newIndex].type)) {
      playerX.value = newX
      playerY.value = newY
      if (COLLECTIBLE_TYPES.includes(grid.value[newIndex].type)) collectGrid(newIndex)
      else if (TRIGGER_TYPES.includes(grid.value[newIndex].type)) {
        if (triggerGrid(newIndex)) return // 领地→战斗，已跳转，跳过后续更新
      }
    }
    updatePlayerPosition()
  }
  ...
  const collectGrid = index => {
    ...
    if (cell.type === 'herb') {
      gain = 5 + rand(16)
      p.spiritHerb = (p.spiritHerb || 0) + gain
      msg = `采集灵草，获得 灵草 ×${gain}`
      ...
      gain = 2 + rand(9)
      p.strengtheningStone = (p.strengtheningStone || 0) + gain
      msg = `开凿矿脉，获得 炼器石 ×${gain}`
    }
    grid.value[index] = { his: '', type: 'empty' }
    if (msg) gameNotifys({ title: '探索收获', message: msg, position: 'top-left', type: 'success' })
  }

  // 触发格：秘境入口 / 怪物领地 / 随机事件
  const triggerGrid = index => {
    const cell = grid.value[index] || {}
    const p = player.value.props
    ...
    if (cell.type === 'portal') {
      // 传送进「秘境探险」玩法（入口保留，可反复进入）
      gameNotifys({ title: '秘境', message: '你踏入秘境入口，进入秘境探险……', position: 'top-left', type: 'info' })
      router.push('/realm')
      return true
    }
    if (cell.type === 'event') {
      const roll = r(3)
      let msg = '你偶得机缘！'
      if (roll === 0) { const g = 300 + r(1500); p.money = (p.money || 0) + g; msg = `获得 灵石 ×${g}` }
      else if (roll === 1) { player.value.cultivation = (player.value.cultivation || 0) + Math.floor(player.value.maxCultivation * 0.05); msg = '修为大涨！' }
      else { const g = 3 + r(10); p.strengtheningStone = (p.strengtheningStone || 0) + g; msg = `获得 炼器石 ×${g}` }
      grid.value[index] = { his: '', type: 'empty' }
      gameNotifys({ title: '奇遇', message: msg, position: 'top-left', type: 'success' })
    }
  }
