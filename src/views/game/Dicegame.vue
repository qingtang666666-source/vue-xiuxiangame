<template>
  <div class="dice-game">
    ...
    <el-button @click="placeBet" :disabled="!canBet" v-if="canBet">下注筹码</el-button>
    <el-countdown :title="hasEnoughMoney ? '冷却中' : '筹码不足'" :value="nextGameTime" @finish="updateCanBet" v-else />
...
<script setup>
  ...
  const placeBet = () => {
    if (!canBet.value || !hasEnoughMoney.value) {
      gameNotifys({ title: '无法下注', message: '筹码不足或正在冷却中' })
      return
    }
  ...
  const finishRoll = () => {
    ...
    const reward = betAmount.value
    canBet.value = false
    gameNotifys({ title: `骰子点数: ${dice}`, message: won ? `恭喜您赢得了${reward}筹码！` : '很遗憾，您输了。' })
