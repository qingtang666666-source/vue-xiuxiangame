  ...
  <div class="endless-tower">
    <div class="battle-info">
      ...
      <div class="monster-info" v-if="monster">
        ...
        <el-tag v-if="towerSup" :type="towerSup > 0 ? 'danger' : towerSup < 0 ? 'warning' : 'info'" size="small" effect="plain" class="sup-tag">
         境界压制：你攻{{ monster.name }} {{ realmSuppressionLabel(towerSup) }} · {{ monster.name }}攻你 {{ realmSuppressionLabel(towerEnemySup) }}
        </el-tag>
        <el-progress
...
<script setup>
  ...
  import { smoothScrollToBottom, formatNumberToChineseUnit, genre, levels, gameNotifys, realmSuppressionPct, realmSuppressionLabel } from '@/plugins/game'
  import { expMult, effectiveBackpackCap, luckMult } from '@/plugins/setBonus'
  ...
  const towerSup = computed(() => (monster.value ? realmSuppressionPct(player.value.level, monster.value.level) : 0))
  const towerEnemySup = computed(() => (monster.value ? realmSuppressionPct(monster.value.level, player.value.level) : 0))
  ...
  const generateCombatLog = (attackerName, defenderName, result) => {
    if (!result.isHit) {
      ...
      // 持续伤害
      if (result.dotDamage > 0) logMessage += `，毒伤/灼伤 ${result.dotDamage} 点`
      // 吸血回复
      if (result.lifesteal > 0) logMessage += `，回复自身 ${result.lifesteal} 气血`
      // 特效触发（麻痹/冰冻/眩晕/中毒/灼烧）
      if (result.effectLogs && result.effectLogs.length) {
        logMessage += `。${result.effectLogs.join('，')}`
      }
  ...
  // 回合制：进行对战 → 弹窗；结果沿用原奖励/失败逻辑
  const openTurnCombat = () => {
    if (player.value.health <= 0) { handlePlayerDefeat(); return }
    if (!monster.value || monster.value.health <= 0) { generateMonster(); return }
    turnShow.value = true
  }
  const onTowerWin = () => { turnShow.value = false; handleMonsterDefeat() }
  const onTowerLose = () => { turnShow.value = false; handlePlayerDefeat() }
  ...
  const sweepFight = () => {
    ...
    // 10% 概率获得装备
    const equipmentGained = Math.random() < 0.1 * luckMult(player.value)
    if (equipmentGained) {
      getRandomEquipment()
