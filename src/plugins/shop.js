...
const shop = {
  drawPrize(lv) {
    ...
    return types.map(({ type, data }) => ({
      type,
      name: genre[type],
      data: data.flatMap((itemName, idx) => {
        const set = pickSetForQuality('pink', Math.floor(Math.random() * 100000))
        return {
          id: Date.now() + idx,
          name: set ? pieceName(set, type, Math.floor(Math.random() * 100000)) : itemName,
          type,
          lock: true,
          level: lv,
