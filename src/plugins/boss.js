...
const boss = {
  drawPrize(lv) {
    ...
    // 血量
    const health = this.getRandomInt(25000, 50000) * lv
    // 攻击
    const attack = this.getRandomInt(2500, 5000) * lv
    // 防御
  ...
  boss_Equip(lv) {
    ...
    for (const [quality, { names, probability }] of Object.entries(weaponTypes)) {
      ...
      if (random < cumulativeProbability) {
        ...
        const Attack = this.getRandomInt(250, 500) * lv * 10
        const Health = this.getRandomInt(2500, 5000) * lv * 10
        const defense = this.getRandomInt(250, 500) * lv * 10
        const Criticalhitrate = this.getRandomFloatInRange(0.05, 0.1)
        const set = pickSetForQuality('danger', Math.floor(Math.random() * 100000))
        return {
          id: Date.now(), // 装备ID
          name: set ? pieceName(set, quality, Math.floor(Math.random() * 100000)) : names[Math.floor(Math.random() * names.length)], //装备名字
          type: quality, // 装备类型
          level: lv, // 装备等级
