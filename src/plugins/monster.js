const monsters = {
  ...
  monster_Attack(lv) {
    ...
    const mult = 1 + s * 0.35
    if (lv <= 144) {
      return Math.floor(this.getRandomInt(25, 75) * lv * mult)
    } else {
      return this.getRandomInt(5000, 25000) * lv
    }
  },
  monster_Health(lv) {
    const s = Math.min(15, Math.max(0, Math.floor((lv - 1) / 9)))
    const mult = 1 + s * 0.35
    if (lv <= 144) {
      return Math.floor(this.getRandomInt(50, 250) * lv * mult)
    } else {
      return this.getRandomInt(5000, 25000) * lv
    }
  },
  monster_Defense(lv) {
    if (lv <= 144) {
      return this.getRandomInt(1, 8) * lv
    } else {
      return this.getRandomInt(250, 500) * lv
    }
  },
