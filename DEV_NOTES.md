...
- `heroPowerOfRank(rank)` = 与玩家同口径的系统加成总和（装备/阵法/功法/宗门/资质/转世/飞升/本命法宝/技艺/境界点），**不设豪杰专用战力上限**；境界点同样享受 ×10，顶级豪杰约 142.4 亿。
...
## 十六、灵宠战力标准（沿用玩家标准体系 ×0.1）

灵宠不再是一套独立的小数字，而是**走玩家同一套境界战力标准体系**：

- 标准：`灵宠战力标准 = realmPower(灵宠境界) × 0.1 × 品质倍率`（`PET_STANDARD_RATIO = 0.1`）。
- 品质倍率：旧版 1 → 1.35（几乎没差距），现改为 **凡 1 / 灵 1.7 / 仙 2.9 / 神 5 / 圣 8.6**，差距放大到 8.6 倍。
- 分配：攻/防/血按定位形状分摊这份战力预算；闪避/暴击按品质上限随境界成长（凡 5% → 圣 28%，`PET_RATE_CAP`）。
  悟性（后天增加的部分）每点 ×1.03、转生每次 ×1.3 继续在标准之上叠加。
- 显示口径统一：界面上的「灵宠战力」= 它给玩家总体实力带来的增量（`petStatsScore` 与 `playerPowerScore` 同一套权重）。
- 实测（`node --import ./tools/preload.mjs tools/check-smoke.mjs`）：

| 品质 | 战力标准(lv144) | 攻 | 血 | 防 | 闪避 / 暴击 |
|---|---|---|---|---|---|
| 凡品 | 1000 万 | 180,890 | 303,896 | 6,512 | 2.8% / 5.0% |
| 灵品 | 1700 万 | 307,509 | 516,616 | 11,070 | 5.5% / 9.0% |
| 仙品 | 2900 万 | 524,578 | 881,291 | 18,884 | 8.3% / 14% |
| 神品 | 5000 万 | 904,452 | 1,519,479 | 32,560 | 11.9% / 20% |
| 圣品 | 8600 万 | 1,555,679 | 2,613,541 | 56,004 | 15.4% / 28% |

- 老档迁移：口径改了之后，已经烤进玩家属性的旧灵宠数值会在启动时换算成新口径
  （`migratePetStatsForPlayer`，在 `App.vue` 挂载时调用；在野灵宠只重算展示值，重复调用是空操作）。
- 调参：只改品质差距 → `PET_QUALITIES[].mult`；只改整体强度 → `PET_STANDARD_RATIO`；只改率上限 → `PET_RATE_CAP`。

## 十七、功法品阶差距（逐阶 ×1.8）

- `TECH_GRADES[].mult`：黄 1 / 玄 1.8 / 地 3.25 / 天 5.85 / 仙 10.5 / 帝 19 / 神 34 / 灵 61 / 皇 110 / 圣 198 / 道 357
  （旧版 1 → 100，一阶只涨 1.5~1.6 倍，品阶差距太小）。
- 影响面：被动功法数值（每重 × 品阶倍率 × 熟练度）、功法售价、神通灵力消耗/强度。
  功法参悟与修炼的**资源、时间成本只按 grade(1~11) 计算**，不随倍率放大，所以高阶功法不会变得更难练。
- 老档无需迁移：功法加成由 `methodStats` 实时结算。

## 十八、手机端功法阁底部显示不全（已修）

- 根因在 `src/components/TechniquePanel.vue`：`.el-drawer__body` 被写成 `height: 100%`，
  而 `el-drawer` 本身是「纵向 flex + overflow: hidden」——body 比「抽屉高度 − 标题栏」更高，
  多出来的那一截被裁掉且滚不到，所以最底部功法卡永远看不全。
- 改为交给 flex 计算高度（`flex: 1 1 auto; min-height: 0; height: auto; box-sizing: border-box`），
  底部 140px 安全边距这才真正可滚出。`/technique` 页面另加了手机端收尾余量。

## 十九、豪杰同步：功法神通 + 灵宠

豪杰（`src/plugins/heroBoard.js`）继续与玩家同口径，这次补上两处：

- **功法/神通**：豪杰的功法本来就取「本境界最高品阶」，品阶倍率放大后自动跟着变强；这次额外让
  **神通的威力随「功法品阶 + 重数」成长**（`pow(品阶倍率, 0.12)` × 重数加成，封顶 ×4.5）。
  例：榜首道阶 20 重神通 ×4.5（原固定 ×1.65），榜尾黄阶 1 重 ×1.3。灵力消耗不变，出手次数不受影响。
- **灵宠**：每位豪杰都带一只灵宠，规则与玩家完全一致（同一套「境界战力标准 ×0.1 × 品质」直出），
  随排名配品质：前 20 圣品 / 前 60 神品 / 前 120 仙品 / 前 200 灵品 / 其余凡品；灵宠境界 = 豪杰境界，
  悟性、转生随排名略增。灵宠三维直接并入豪杰属性（与玩家佩戴灵宠一致），因此同样会被阵法/符箓的百分比放大。
- 展示：豪杰榜悬浮提示补上「功法 · 神通 · 灵宠」三行；面板抽屉宽度改为 `min(420px, 100vw)`，手机端不再超屏。
- 实测（`node --import ./tools/preload.mjs` 现算）：榜首 153.1 亿（原 142.6 亿），其中灵宠面板战力 1.93 亿；
  榜尾 36.2 万，人人都有功法与灵宠。

## 二十、手机端强化面板（已修）

- `src/components/StrengthenPanel.vue`：抽屉原先没设宽度（el-drawer 默认 30%，手机上只剩一条竖缝），
  改为 `size="min(440px, 100vw)"`；body 同样改成 flex 高度 + 底部安全区，避免最后一截被裁掉。
- 操作区（`点击强化` / `点击精炼`，受损时的「修复装备」）改成**常驻底部**（`position: sticky; bottom: 0`），
  手机端不用滚到底也能看到并点到强化按钮。

## 二十一、战斗灵力随境界提升

旧规则：灵力池 = `max(150, 上阵神通总耗 × 1.2)`，跟境界完全无关（只间接跟着功法品阶涨）。

- 新规则（`battleEngine.battleMpPool`）：`灵力 = max(境界底子, 上阵神通总耗 × 轮数)`
  - **轮数**：把上阵神通各放一遍算一轮 —— 低境界 1.2 轮 → 道祖 2.2 轮（`MP_CYCLE_MIN/MAX`）。
  - **境界底子**：150 → 600（1 级到道祖，`MP_FLOOR_MIN/MAX`），少带/没带神通时也有基本灵力。
  - 依旧不跟气血/战力膨胀，只跟「境界 × 实际神通消耗」走。
- 实测可放次数：1 级 6 次 → 60 级 8 次 → 100 级 9 次 → 道祖 11 次（同一套道阶神通）。
  纯底子（不带神通）：lv1 150 → lv144 600。
- **豪杰同步**：`heroEnemy` 现在按自己境界算灵力池（`battleMpPool(境界, 5×神通消耗)`，豪杰满配 5 门主动功法），
  并由 `monsterToEntity` 优先采用怪物自带的 `maxMp`（普通野怪仍用旧公式，不受影响）。
  实测：榜尾 312（6 次）→ 榜中 486（9 次）→ 榜首 600（14 次），随境界递增。
